...
def main(args): 
    ...
    audio_files = list(Path(args['input_directory']).rglob('*.wav')) + \
                  list(Path(args['input_directory']).rglob('*.WAV'))
    ...
    for ii, ip_path in enumerate(ip_files):
        sampling_rate, ip_audio = au.load_audio_file(ip_path, args['time_expansion_factor'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('input_directory', type=str, help='Input directory containing the audio files')
    parser.add_argument('output_directory', type=str, help='Output directory the clipped audio files')
    parser.add_argument('--output_duration', default=2.0, type=float, help='Length of output clipped file (default is 2 seconds)')
