...
from flask import Flask, session, request, render_template, redirect, copy_current_request_context
import uuid
...
def initialize_session(file_index=0):
    ...
    session['file_index'] = file_index  # int, keeps track of the index of the currently selected file
...
def submit_annotations():
    ...
    if len(request.form) > 0 and json.loads(request.form['change_made']):
        ...
        dataset['annotations'][input_id]['annotation'] = json.loads(request.form['updated_annotations'])
        ...
        this_file_class_list = set([aa['class'] for aa in dataset['annotations'][input_id]['annotation']])
        sorted_classes = sorted(list(this_file_class_list.union(set(dataset['class_names'])) - set(config.CLASS_NAMES)))
    ...
    next_id = select_file(len(dataset['file_names']), file_id=id, next_file=True)
    url_str = '/annotate/?file_name=' + dataset['file_names'][next_id] + '&dataset_id=' + session['dataset_id']
...
def render_annotation_page():
    ...
    if ('dataset_id' in request.args and request.args['dataset_id'] in datasets.keys()):
    ...
    if ('file_name' in request.args and request.args['file_name'] in dataset['file_names']):
    ...
    return render_template("annotate.html", annotation=annotation, file_index=session['file_index'],
                           next_file=next_file, prev_file=prev_file, dataset_id=session['dataset_id'],
                           class_names=dataset['class_names'], event_names=dataset['event_names'],
                           im_data=im_data, aud_data=aud_data, file_params=file_params,
                           annotations=annotations_sorted, annotation_dir=dataset['annotation_dir'],
                           audio_dir=dataset['audio_dir'], spectrogram_params=get_spectrogram_params())
...
def render_dataset_list_page():
    ...
    dataset_info = []
    for dd in datasets:
        dataset_info.append({'dataset_id':dd,
...
def render_file_list_page():
    ...
    return render_template("file_list.html", annotations=annotations_sorted,
                           dataset_id=session['dataset_id'], annotation_dir=annotation_dir,
                           audio_dir=audio_dir)
...
def set_spectrogram_params():
    ...
    if request_val('win_length_units') is not None:
        session['spectrogram_params']['win_length_units'] = request_val('win_length_units')
        session.modified = True

    if session['spectrogram_params']['win_length_units'] == 'samples':
        session['spectrogram_params']['fft_win_length'] = int(session['spectrogram_params']['fft_win_length'])
        session.modified = True

    url_str = '/annotate/?file_name=' + request_val('file_name') + '&dataset_id=' + request_val('dataset_id')
    return redirect(url_str, code=302)
...
def get_data(annotation, use_cache=True):
    ...
    if use_cache:
        ...
        update_cache(annotation['file_name'], dataset['file_names'], dataset['annotations'], dataset['audio_dir'])
...
def gen_cache_key(file_name, spec_params):
    ...
    param_string = '&'.join(list('{}={}'.format(key, value) for key, value in spec_params.items()))
...
def update_cache(file_id, file_names, annotations, audio_dir):
    ...
    for f_id in file_indexes_for_cache:
        if gen_cache_key(file_names[f_id], get_spectrogram_params()) not in cache.keys():
    ...
    cache_data = [{k: item[k] for k in ('cache_key', 'created_at')} for item in cache.values()]
...
def compute_data(annotation, audio_dir):
    ...
    sampling_rate, audio_raw, aud_data, duration = gd.compute_audio_data(annotation, audio_dir, tm_exp_play)
    im_data, im_shape = gd.compute_image_data(audio_raw, sampling_rate, get_spectrogram_params())
    ...
    if get_spectrogram_params()['win_length_units'] == 'seconds':
        ...
        win_length_secs = float(get_spectrogram_params()['fft_win_length']) / sampling_rate


    file_params = {'fft_win_length_secs': win_length_secs,
                   'fft_overlap': get_spectrogram_params()['fft_overlap'],
                   'spec_height': im_shape[0], 'spec_width': im_shape[1],
...
def check_files_and_annotations():
    ...
    if len(dataset['file_names']) == 0 or (len(dataset['annotations']) != len(dataset['file_names'])):
...
def create_dataset(audio_dir, annotation_dir):
    ...
    audio_files = list(Path(audio_dir).rglob('*.wav')) + list(Path(audio_dir).rglob('*.WAV'))
    audio_files = [os.path.join(aa.parent, aa.name) for aa in audio_files]
    if len(audio_files) == 0:
        return 'none'

    # create paths for annotation files
    ann_file_paths = [aa.replace(audio_dir, annotation_dir) + '.json' for aa in audio_files]
    class_names = []
    event_names = []
    annotations = {}
    ...
    dataset['event_names'] = config.EVENT_NAMES + sorted(list(set(event_names) - set(config.EVENT_NAMES)))
    dataset['class_names'] = config.CLASS_NAMES + sorted(list(set(class_names) - set(config.CLASS_NAMES)))
